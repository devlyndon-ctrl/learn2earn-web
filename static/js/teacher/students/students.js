...
// Function to populate classroom dropdown with teacher's assigned classrooms - UPDATED FOR ASCENDING ORDER
    ...
    fetch('/api/teacher-classrooms')
        ...
        .then(data => {
            if (data.success && data.classrooms) {
                // Sort classrooms by grade level (ascending), then by section (ascending)
                const classrooms = data.classrooms.sort((a, b) => {
                    // First sort by grade level (numeric)
                    const gradeCompare = parseInt(a.grade_level) - parseInt(b.grade_level);
                    if (gradeCompare !== 0) return gradeCompare;
                    
                    // If same grade level, sort by section (alphabetical)
...
// Updated function to populate student dropdown based on selected classroom - UPDATED FOR ASCENDING ORDER
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initializePage() {
        ...
        console.log('%c📡 STUDENTS PAGE: Calling /api/my-students', 'color: #00D4FF; font-weight: bold;');
        fetch('/api/my-students')
            .then(res => {
                console.log(`📡 STUDENTS PAGE: API responded with status ${res.status}`);
                if (!res.ok) {
            ...
            .then(data => {
                console.log('%c✅ STUDENTS PAGE: Raw API Response:', 'color: #00FF40; font-weight: bold;', data);
                console.log('%c📊 STUDENTS PAGE: Data Structure:', 'color: #FFC542; font-weight: bold;', {
                    success: data.success,
                    classrooms_count: data.classrooms ? data.classrooms.length : 0,
                    classrooms_data: data.classrooms,
                    students_count: data.students ? data.students.length : 0,
                    students_sample: data.students ? data.students.slice(0, 3) : [],
                ...
                if (data.success) {
                    ...
                    console.log(`%c📚 STUDENTS PAGE: Loaded ${window.allStudents.length} students, ${data.classrooms ? data.classrooms.length : 0} classrooms`, 'color: #5E72E4; font-weight: bold;');
            ...
            .catch(error => {
                console.error('%c❌ STUDENTS PAGE: Error fetching students:', 'color: #FF0000; font-weight: bold;', error);
    ...
    function paginateStudents(students) {
        ...
        // Sort students by points in descending order (highest first)
        ...
        const studentsToDisplay = sortedStudents.slice(startIndex, endIndex);
...
<td class="text-center align-middle">
    ...
    >
        ...
        // Show/hide pagination based on whether there are students
    ...
    // Classroom filter for main table - UPDATED FOR ASCENDING ORDER
    function populateClassroomFilter(classrooms) {
        ...
        const sortedClassrooms = classrooms.sort((a, b) => {
            // First sort by grade level (numeric)
            const gradeCompare = parseInt(a.grade_level || a.grade) - parseInt(b.grade_level || b.grade);
            if (gradeCompare !== 0) return gradeCompare;
        ...
        sortedClassrooms.forEach(c => {
            select.innerHTML += `<option value="${c.grade_level || c.grade}|${c.section}">${c.label}</option>`;
        });
        select.disabled = sortedClassrooms.length === 0;
    ...
    document.getElementById('exportStudentExcelBtn')?.addEventListener('click', function() {
        ...
        // Sort by section (ascending), then by name (ascending)
    ...
    if (refreshBtn) {
        refreshBtn.addEventListener('click', function() {
            ...
            refreshBtn.querySelector('i').classList.add('fa-spin'); // Add spinning animation
            
            // Call initializePage and remove spin after a delay
            initializePage();
            setTimeout(() => {
    ...
    // ============= ASSIGNMENT CHANGE LISTENERS & AUTO-REFRESH =============
    // Make initializePage available globally for manual calls
    window.loadStudents = initializePage;

    // Listen for storage changes (different tabs)
    window.addEventListener('storage', function(e) {
        if (e.key === 'teacherAssignmentsUpdated') {
            console.log('📢 Assignment change detected in another tab, refreshing...');
            setTimeout(initializePage, 100); // Small delay to ensure data is written
        }
    });

    // Listen for custom events (same tab/window)
    window.addEventListener('assignmentChanged', function(e) {
        console.log('📢 Assignment change event received, refreshing...');
        setTimeout(initializePage, 100);
    });

    // Listen for page visibility changes - refresh when user returns to tab
    document.addEventListener('visibilitychange', function() {
        if (document.visibilityState === 'visible') {
            console.log('👁️ Page is now visible, checking for updates...');
            setTimeout(initializePage, 100);
        }
    });

    // Initialize the page immediately - no complex waiting
    initializePage();
});
...
function debugPageState() {
    ...
    if (isAdminSubject) {
        console.log('Teachers count:', Object.keys(window.teachersData || {}).length);
        console.log('Current assignments:', (window.currentAssignments || []).length);
        console.log('Current school year:', window.currentSchoolYear);
        console.log('Assignment filtered data:', (window.assignmentFilteredData || []).length, 'rows');
    } else if (isStudents) {
